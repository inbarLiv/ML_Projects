...
def sequential_forward_selection(
    ...
    def create_select_indicator(df_data_train: pd.DataFrame, list_ls: list) -> list:
        ...
        sum_col = pd.Series(df.iloc[:, 2:len(list_ls) + 2].sum(axis=1), name='sum_ind')
        sum_rank = pd.Series(df_rank.iloc[:, 2:len(list_ls) + 2].sum(axis=1), name='sum_rank')
...
def cfg_fs(train_dir: str, min_to_select: int, top_univariate: int,top_multivariate: int,
    ...
    :param min_to_select: minimum features that will be selected
    :param top_univariate: top x features to select based on ranking following the univariate selection stage
    :param top_multivariate: top x features to select based on ranking following the multivariate selection stage
    :param folds_num_tree: how many folds to run the cv with, in the trees feature selection stage
...
top_univariate: int = 50
top_multivariate: int = 30
